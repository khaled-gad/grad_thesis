...
GEMM computes $C = A \times B$, where $A$ is $M \times K$, $B$ is $K \times N$, and the result $C$ is $M \times N$. Each output element represents a dot product:
...
\begin{lstlisting}[language=Python, caption={Scalar GEMM pseudo-code}]
...
The vectorized code replaces the innermost column loop with a while loop that processes columns in chunks. We track how many columns remain to be processed and handle them in groups of $vl$:

\begin{lstlisting}[language=Python, caption={Vectorized GEMM pseudo-code}]
for each row i in matrix A:
    remaining_columns = N
    while remaining_columns > 0:
        vl = set_vector_length(remaining_columns)
        j = N - remaining_columns
        
        acc_vector = broadcast(0, vl)
        for k from 0 to K - 1:
            # Broadcast scalar from A to all vector lanes
            a_scalar = A[i][k]
            # Load vl contiguous elements from B
            b_vector = load_vector(B[k][j], vl)
            
            # Vector-Scalar Fused Multiply-Add
            acc_vector = fma(acc_vector, a_scalar, b_vector)
            
        store_vector(C[i][j], acc_vector, vl)
...
\begin{lstlisting}[language=Python, caption={Scalar Dense Layer pseudo-code}]
for each output neuron j from 0 to N - 1:
    acc = bias[j]
    for each input feature k from 0 to K - 1:
...
We parallelize across output neurons, computing $vl$ neurons simultaneously. The key insight is that for each input feature, we can broadcast that feature value and multiply it with weights from multiple neurons in parallel.
...
\begin{lstlisting}[language=Python, caption={Vectorized Dense Layer pseudo-code}]
j = 0
while j < N:
    vl = set_vector_length(N - j)
    # Sequential load of bias values
    acc_vector = load_vector(bias + j, vl)
    
    for each input feature k from 0 to K - 1:
        # Load weights for vl different neurons (strided access)
        # Stride is equal to the row width K
        weight_vector = strided_load(weights + (j * K) + k, stride=K, vl)
        
        # Broadcast the scalar input feature
        input_val = input[k]
        
        # Vector-Scalar Fused Multiply-Add
        acc_vector = fma(acc_vector, input_val, weight_vector)
        
    store_vector(output + j, acc_vector, vl)
    j += vl
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption={Scalar 2D Convolution pseudo-code}]
...
\begin{lstlisting}[language=Python, caption={Vectorized 2D Convolution pseudo-code}]
Repack kernel so output channels are contiguous
Initialize output to zero

for each batch b:
    for output channels oc in vector chunks:
        vl = set_vector_length(remaining_channels)
        for each output row oh, output column ow:
            # Load initial accumulators for vl channels
            acc_vector = load_vector(output[b][oc][oh][ow], vl)
                ...
                for each kernel row kh:
                    for each kernel column kw:
                        ...
                        if ih, iw inside input bounds:
                            ...
                            # Load weights for vl consecutive output channels
                            weight_vec = load_vector(packed_w[ic][kh][kw][oc], vl)
                            # Fused Multiply-Add (Scalar * Vector)
                            acc_vector = fma(acc_vector, input_val, weight_vec)
            
            store_vector(output[b][oc][oh][ow], acc_vector, vl)
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption={Im2Col Transformation pseudo-code}]
Compute output height and width
...
for each output position (oh, ow):
    for each input channel ic:
        for each kernel position kh, kw:
            input_val = get_input_with_padding(b, ic, oh, ow, kh, kw)
            col[K_index][oh * out_w + ow] = input_val

# Step 2: GEMM
gemm_output = matrix_multiply(kernel_matrix, col_matrix)

# Step 3: Bias Add
for each output channel oc:
    bias_vec = broadcast(bias[oc])
    for each output position in chunks:
        out_vec = load_vector(gemm_output[oc][pos])
        out_vec = out_vec + bias_vec
        store_vector(output[oc][pos], out_vec)
\end{lstlisting}
...
\subparagraph{Specialized 3x3 Vectorization}

The 3x3 kernel size deserves special attention because it dominates modern CNN architectures. When we know the kernel size is fixed at 3x3, we can apply optimizations that wouldn't be practical for variable-size kernels.
...
\begin{lstlisting}[language=Python, caption={Specialized 3x3 Convolution pseudo-code}]
for each output row oh:
    # Preload input rows for the current sliding window set
    row0 = input[b][ic][oh]
    row1 = input[b][ic][oh+1]
    row2 = input[b][ic][oh+2]
    
    ow = 0
    while ow < output_width:
        vl = set_vector_length(output_width - ow)
        # Load vectors from preloaded rows at different horizontal offsets
        v00 = load_vector(row0 + ow, vl)
        v01 = load_vector(row0 + ow + 1, vl)
        v02 = load_vector(row0 + ow + 2, vl)
        
        v10 = load_vector(row1 + ow, vl)
        v11 = load_vector(row1 + ow + 1, vl)
        v12 = load_vector(row1 + ow + 2, vl)
        
        v20 = load_vector(row2 + ow, vl)
        v21 = load_vector(row2 + ow + 1, vl)
        v22 = load_vector(row2 + ow + 2, vl)
        
        # Accumulate 9 FMA operations for the 3x3 window
        acc = v00 * k00 + v01 * k01 + v02 * k02
        acc = acc + v10 * k10 + v11 * k11 + v12 * k12
        acc = acc + v20 * k20 + v21 * k21 + v22 * k22
        
        store_vector(output[oc][oh][ow], acc, vl)
        ow += vl
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption={Scalar Max Pooling pseudo-code}]
Compute output height and width
    ...
    for each channel c:
        for each output row oh:
            for each output column ow:
                ...
                max_val = -infinity
                for h from h_start to h_end - 1:
                    for w from w_start to w_end - 1:
...
We vectorize across output columns, computing $vl$ output positions simultaneously. Each vector lane maintains an independent maximum accumulator. As we scan the pooling window, we load vectors of input values and compute element-wise maximums, updating all lanes in parallel.
...
\begin{lstlisting}[language=Python, caption={Vectorized Max Pooling pseudo-code}]
Compute output height and width
...
for each batch b:
    for each channel c:
        for each output row oh:
            ...
            while ow < output_width:
                vl = set_vector_length(output_width - ow)
                max_vector = broadcast(-infinity, vl)
                
                for kh from 0 to k_h - 1:
                    ih = ih_start + kh
                    if ih < 0 or ih >= input_height:
                        continue
                        
                    for kw from 0 to k_w - 1:
                        # Input positions are separated by stride_w
                        iw_start = ow * stride_w - pad_w + kw
                        input_vector = strided_load(input_ptr + iw_start, stride_w, vl)
                        max_vector = max(max_vector, input_vector)
                
                store_vector(output_ptr + ow, max_vector, vl)
                ow += vl
\end{lstlisting}
...
The inner loops iterate through the pooling window. For each kernel row, we check if the corresponding input row is within bounds. If not, we skip this entire row (the \texttt{continue} statement). For each kernel column, we compute the starting input column position and load $vl$ consecutive input values.

This load is crucial: for output column $ow$, we need input at position $ow \times \text{stride}_w - \text{pad}_w + kh$. For the next output column $ow+1$, we need input at position $(ow+1) \times \text{stride}_w - \text{pad}_w + kh$. These positions are separated by \texttt{stride\_w} in the input. So when stride is 1, we load consecutive elements; when stride is 2, we load every other element; and so on.

The RISC-V vector ISA can handle this through either strided loads (when stride more than 1) or indexed loads. The notation \texttt{strided\_load} abstracts this detail, but the actual implementation would use the appropriate load instruction variant.
...
\end{itemize}
% It is left in parancethes? should not be just low-risk, high-payoff 
...
% ============================================================
% PATTERN 4: Tensor Indexing and Data Movement
% ============================================================
\subsubsection{Pattern 4: Tensor Indexing and Data Movement}
...
% ------------------------------------------------------------
\paragraph{Gather and Gather Elements}

\subparagraph{Kernel Description}

Gather operations collect data from an input tensor based on a provided index tensor. While a standard Gather typically selects entire slices along an axis, GatherElements performs a more granular, element-wise selection. For a 2D tensor along $axis=1$, the operation is defined as:
\[ \text{output}[i][j] = \text{data}[i][\text{indices}[i][j]] \]

\subparagraph{Scalar Implementation}

The scalar approach requires nested loops to resolve indices one by one. For each output position, the processor must calculate a new memory address, perform a load, and then store the result, often leading to pipeline stalls during the address resolution phase:

\begin{lstlisting}[language=Python, caption={Scalar Gather Elements (Axis 1) pseudo-code}]
for i from 0 to indices_rows - 1:
    for j from 0 to indices_cols - 1:
        # Resolve index for the target axis
        target_col = indices[i][j]
        output[i][j] = data[i][target_col]
\end{lstlisting}

\subparagraph{Vectorization Strategy}

Vectorization is achieved by processing a chunk of indices simultaneously. The key challenge in RVV is that indices are often provided as 64-bit integers, while data elements (like floats) are 32-bit. We utilize narrowing operations to convert 64-bit indices into 32-bit offsets. For axis-based gathers, we calculate byte offsets by scaling the indices by the data stride. These are then passed to a vector indexed-load instruction (\texttt{vluxei}), which fetches non-contiguous data into a single vector register.

\subparagraph{Implementation}

The vectorized implementation leverages \texttt{vsetvli} to handle index chunks and narrowing wrappers to align data types:

\begin{lstlisting}[language=Python, caption={Vectorized Gather Elements pseudo-code}]
for i from 0 to indices_rows - 1:
    j = 0
    while j < indices_cols:
        vl = set_vector_length(indices_cols - j)
        
        # Load 64-bit indices and narrow to 32-bit
        v_idx64 = load_vector(indices[i * indices_cols + j], vl)
        v_idx32 = narrow_to_32bit(v_idx64, vl)
        
        # Calculate byte offsets (index * element_size)
        v_offsets = v_idx32 << 2  # Shift left by 2 for float32 (4 bytes)
        
        # Indexed load and contiguous store
        v_vals = indexed_load(data_row_ptr, v_offsets, vl)
        store_vector(output[i * indices_cols + j], v_vals, vl)
...
Scatter is the inverse of gather; it writes values from an updates tensor into a data tensor at positions specified by indices. For $axis=0$ (rows), the output is updated as:
\[ \text{output}[\text{indices}[i][j]][j] = \text{updates}[i][j] \]

\subparagraph{Scalar Implementation}

The scalar approach requires a full copy of the original data followed by individual updates. This involves heavy branching and individual store instructions that cannot be pipelined easily, as each store address is potentially in a completely different memory bank.

\begin{lstlisting}[language=Python, caption={Scalar Scatter Elements pseudo-code}]
output = copy(data) # Initial pass to preserve un-updated elements

for i from 0 to indices_rows - 1:
    for j from 0 to indices_cols - 1:
        target_row = indices[i][j]
        output[target_row][j] = updates[i][j]
\end{lstlisting}

\subparagraph{Vectorization Strategy}

Similar to Gather, we vectorize the update loop. When scattering along rows ($axis=0$), the target addresses are separated by the row stride ($W \times \text{sizeof(float)}$). We compute absolute byte offsets by combining the narrowed indices with a Vector ID (\texttt{vid}) multiplied by the element size to account for the current column position. This allows us to perform an unordered indexed store (\texttt{vsuxei}) for the entire vector chunk.

\subparagraph{Implementation}

The vectorized implementation focuses on efficient address generation for the scattered writes:

\begin{lstlisting}[language=Python, caption={Vectorized Scatter Elements (Axis 0) pseudo-code}]
output = copy(data)
...
for i from 0 to indices_rows - 1:
    ...
    while j < indices_cols:
        ...
        v_idx64 = load_vector(indices[i * indices_cols + j], vl)
        v_updates = load_vector(updates[i * indices_cols + j], vl)
        v_idx32 = narrow_to_32bit(v_idx64, vl)
        
        # Calculate row-stride offsets: (index * data_cols * 4)
        v_row_byte = v_idx32 * (data_cols * 4)
        
        # Add column-wise byte offsets: (j + vector_id) * 4
        v_col_idx = get_vector_id() + j
        v_col_byte = v_col_idx * 4
        v_final_off = v_row_byte + v_col_byte
        
        # Vector indexed store (Scatter)
        indexed_store(output, v_final_off, v_updates, vl)
...
% ============================================================
% PATTERN 5: Batch Normalization
% ============================================================
\subsubsection{Pattern 5: Batch Normalization}
...
For each channel $c$, Batch Normalization transforms the input $x$ using the learned mean ($\mu$), variance ($\sigma^2$), scale ($\gamma$), and shift ($\beta$) parameters:
...
The scalar approach processes the spatial dimensions sequentially for each channel. This results in significant overhead as the processor must manage loop counters and individual memory requests for every pixel, failing to exploit the contiguous nature of the data:

\begin{lstlisting}[language=Python, caption={Scalar Batch Normalization pseudo-code}]
for c from 0 to channels - 1:
    # Pre-calculate constants for the current channel
    alpha = scale[c] / sqrt(variance[c] + epsilon)
    beta_prime = bias[c] - mean[c] * alpha
    
    for i from 0 to spatial_dim - 1:
        output[c][i] = input[c][i] * alpha + beta_prime
\end{lstlisting}

\subparagraph{Vectorization Strategy}

We vectorize across the spatial dimensions ($H \times W$) for each channel. Since the data within a channel is stored contiguously in memory (unit-stride), we can maximize memory bus utilization. For each channel, we broadcast the pre-calculated $\alpha$ and $\beta'$ values across all vector lanes. We then iterate through the spatial data in chunks of $vl$, performing a vector-scalar FMA. This approach amortizes the cost of the channel-specific parameter calculations over thousands of spatial elements.

\subparagraph{Implementation}

The vectorized implementation utilizes unit-stride loads and stores, combined with vector-scalar arithmetic to achieve near-peak memory bandwidth:

\begin{lstlisting}[language=Python, caption={Vectorized Batch Normalization pseudo-code}]
spatial_dim = height * width

for c from 0 to channels - 1:
    # Calculate transformation constants per channel
    alpha = scale[c] / sqrt(variance[c] + epsilon)
    beta_prime = bias[c] - mean[c] * alpha
    
    i = 0
    while i < spatial_dim:
        vl = set_vector_length(spatial_dim - i)
        
        # Load contiguous input segment
        v_in = load_vector(input_ptr + (c * spatial_dim) + i, vl)
        
        # Vector-Scalar Fused Multiply-Add: (v_in * alpha) + beta_prime
        v_out = fma(v_in, alpha, beta_prime, vl)
        
        # Unit-stride store
        store_vector(output_ptr + (c * spatial_dim) + i, v_out, vl)
        
        i += vl
\end{lstlisting}

By hoisting the $\alpha$ and $\beta'$ calculations outside the inner loop, we reduce the computational cost to a single FMA per element. The use of \texttt{vsetvli} ensures that the kernel remains efficient regardless of the feature map size, handling the final "tail" of the spatial dimension without the need for manual scalar cleanup loops.

% ============================================================
% PATTERN 6: Post-Processing Kernels (NMS)
% ============================================================
\subsubsection{Pattern 6: Post-Processing Kernels - Non-Maximum Suppression}
...
The IoU calculation benefits significantly from vectorization when computing IoU between one box and multiple candidate boxes simultaneously:
