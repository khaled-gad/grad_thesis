...
While Spike is the official RISC-V ISA simulator, QEMU offers several advantages aligned with the goals of this project. QEMU supports full Linux execution environments, enabling realistic testing of vectorized kernels in the presence of system calls, memory management, and runtime libraries. Additionally, QEMU integrates seamlessly with GNU Debugger (GDB), allowing interactive debugging and inspection of vector register state.
% did we ever make use of this? should this ever be mentioned? ISA-level validation?
...
\begin{figure}[H]
    ...
    \caption[ONNX Model Computational Graph]{ONNX Model Computational Graph example illustrating nodes (operators) and edges (tensor data flow).}
